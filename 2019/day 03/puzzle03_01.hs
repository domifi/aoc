...
main = do
    ...
    let parsedInput = map (wirePathToCoords . (map readWirePath . splitComma)) . lines $ input



    print . minimum . map (manDist (0,0)) . intersectLists $parsedInput
    --print . map (wirePathToCoords . (map readWirePath . splitComma)) . lines $ input
